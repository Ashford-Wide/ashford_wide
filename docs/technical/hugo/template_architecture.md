...
> [!NOTE]
> `partials/jsonld/closures.html` also exists in the repo but is not called from any template — it's dead code left over from an earlier version of the road closures page. See [docs/technical/known_gaps_future_work.md](../known_gaps_future_work.md).
...
| `shortcodes/image.html` | Wraps `partials/image.html` for use directly in Markdown content — see [docs/technical/hugo/images.md](images.md) |
...
| `shortcodes/road-closure-map.html` | Leaflet map of Remembrance Sunday road closures — see [docs/technical/remembrance_map.md](../remembrance_map.md) |
...
Note: the homepage member logo marquee (`layouts/index.html`) is driven by `site.Data.businesses` filtered to entries with a `logo` set. There is no `data/members.yaml` file or CMS collection — see [docs/technical/hugo/data_files.md](data_files.md).
...
`business-member` is not currently a managed collection in `static/admin/config.yml` — the `pages` collection only reaches top-level files directly in `content/`, which does not include `content/business-member/` (see [docs/editorial/cms_collections.md](../../editorial/cms_collections.md)). Adding a dedicated Sveltia collection for this content type would need a new `folder` collection entry mirroring the `events`/`news` pattern.

> [!CAUTION]
> `content/business-member/` currently contains no pages — the one example page that used to live here (`timeless-ims.md`) was created and then deleted, and no business currently has a dedicated profile page. See [docs/technical/known_gaps_future_work.md](../known_gaps_future_work.md).

See [docs/editorial/business_pages.md](../../editorial/business_pages.md) for front matter fields, page structure, and how to add a new one.
