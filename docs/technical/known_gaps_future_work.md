...
| **No `data/members.yaml`** | Some older code comments/docs referenced a members data file distinct from the business directory; it never existed. The homepage member logo marquee reads from `data/businesses.yaml` instead — see [docs/technical/hugo/data_files.md](hugo/data_files.md). |
