...
| File | Purpose |
|---|---|
| `static/closures.geojson` | Road geometry data |
...
4. Replace `static/closures.geojson` with the new file.
...
The shortcode is self-contained. It loads Leaflet from the [unpkg](https://unpkg.com/) CDN, renders a map `<div>`, then fetches `/closures.geojson` and adds the features as styled polylines.
...
- **Popups** — clicking a line shows `feature.properties.name` (the OSM road name).
- **Style** — dashed red lines (`color: #cc0000`, `weight: 7`, `dashArray: '10, 10'`) to indicate a closure.
...
| `fetch('/closures.geojson')` | `connect-src` | `'self'` |
